...
router.post('/connect-simple', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const scopes = ''; // Vazio - deixa o Facebook decidir permissões básicas
    ...
    try {
        ...
        if (isSimplifiedFlow && instagramAccounts.length > 0 && userId) {
            try {
                ...
                const existing = await (0, connection_1.query)('SELECT id FROM instagram_integrations WHERE user_id = ? AND instagram_account_id = ?', [userId, firstAccount.id]);
                if (existing.rows.length > 0) {
                    // Update existing
                    await (0, connection_1.query)(`UPDATE instagram_integrations 
