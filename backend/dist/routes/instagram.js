...
router.post('/connect-simple', auth_1.authenticate, async (req, res) => {
    try {
...
router.post('/connect', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const existing = await (0, connection_1.query)('SELECT id FROM instagram_integrations WHERE user_id = ? AND instagram_account_id = ?', [userId, instagram_account_id]);
        let integrationId;
        if (existing.rows.length > 0) {
            // Update existing integration
            integrationId = existing.rows[0].id;
            await (0, connection_1.query)(`UPDATE instagram_integrations 
         ...
         WHERE id = ?`, [
            ...
            const result = await (0, connection_1.query)(`INSERT INTO instagram_integrations 
        ...
        const integration = await (0, connection_1.query)('SELECT * FROM instagram_integrations WHERE id = ?', [integrationId]);
        res.status(201).json({
            success: true,
...
router.get('/callback', async (req, res) => {
    try {
        ...
        let tokenResponse;
        try {
            tokenResponse = await axios_1.default.get(`${INSTAGRAM_GRAPH_API_BASE}/oauth/access_token`, {
                params: {
                    client_id: INSTAGRAM_APP_ID,
        ...
        try {
            accountsResponse = await axios_1.default.get(`${INSTAGRAM_GRAPH_API_BASE}/me/accounts`, {
                params: {
        ...
        for (const page of pages) {
            if (page.instagram_business_account) {
                try {
                    const igAccountResponse = await axios_1.default.get(`${INSTAGRAM_GRAPH_API_BASE}/${page.instagram_business_account.id}`, {
                        params: {
        ...
        if (isSimplifiedFlow && instagramAccounts.length > 0 && userId) {
            try {
                ...
                const existing = await (0, connection_1.query)('SELECT id FROM instagram_integrations WHERE user_id = ? AND (instagram_account_id = ? OR (instagram_username = ? AND instagram_account_id LIKE ?))', [userId, firstAccount.id, accountUsername, 'personal_%']);
                if (existing.rows.length > 0) {
                    // Update existing
                    await (0, connection_1.query)(`UPDATE instagram_integrations 
             ...
             WHERE id = ?`, [
                    ...
                    // Create new
                    await (0, connection_1.query)(`INSERT INTO instagram_integrations 
             (user_id, title, access_token, instagram_account_id, instagram_username, expires_at, status)
             VALUES (?, ?, ?, ?, ?, ?, 'active')`, [
                        userId,
                        title,
...
router.get('/list', auth_1.authenticate, async (req, res) => {
    try {
        const userId = (req.user && req.user.id);
        const integrations = await (0, connection_1.query)(`SELECT id, title, instagram_account_id, instagram_username, status, created_at, updated_at 
...
router.get('/:integrationId', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const userId = (req.user && req.user.id);
        const integration = await (0, connection_1.query)(`SELECT id, title, instagram_account_id, instagram_username, status, created_at, updated_at 
       FROM instagram_integrations 
...
router.put('/:integrationId', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const userId = (req.user && req.user.id);
        // Verify ownership
        const existing = await (0, connection_1.query)('SELECT id FROM instagram_integrations WHERE id = ? AND user_id = ?', [integrationId, userId]);
        if (existing.rows.length === 0) {
            return res.status(404).json({ message: 'Integração não encontrada' });
        }
        const updates = [];
       ...
       SET ${updates.join(', ')} 
       WHERE id = ?`, [...values]);
        const updated = await (0, connection_1.query)('SELECT * FROM instagram_integrations WHERE id = ?', [integrationId]);
        res.json({
            success: true,
...
router.delete('/:integrationId', auth_1.authenticate, async (req, res) => {
    try {
        ...
        await (0, connection_1.query)('DELETE FROM instagram_integrations WHERE id = ?', [integrationId]);
        res.json({
            success: true,
...
router.post('/:integrationId/refresh', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const userId = (req.user && req.user.id);
        // Get integration
        const integration = await (0, connection_1.query)('SELECT * FROM instagram_integrations WHERE id = ? AND user_id = ?', [integrationId, userId]);
        if (integration.rows.length === 0) {
            return res.status(404).json({ message: 'Integração não encontrada' });
        }
        ...
        const tokenResponse = await axios_1.default.get(`${INSTAGRAM_GRAPH_API_BASE}/oauth/access_token`, {
            params: {
...
router.get('/accounts', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // Get user's pages
        const pagesResponse = await axios_1.default.get(`${INSTAGRAM_GRAPH_API_BASE}/me/accounts`, {
            params: {
                access_token: access_token,
                fields: 'id,name,access_token,instagram_business_account'
            }
...
router.get('/:integrationId/insights', auth_1.authenticate, async (req, res) => {
    try {
        const { integrationId } = req.params;
        const userId = (req.user && req.user.id);
        ...
        try {
            const metrics = metric ? metric.split(',') : ['impressions', 'reach', 'profile_views', 'website_clicks'];
            const insightsResponse = await axios_1.default.get(`${INSTAGRAM_GRAPH_API_BASE}/${instagram_account_id}/insights`, {
                params: {
                    access_token: access_token,
                    metric: metrics.join(','),
                    period: period,
