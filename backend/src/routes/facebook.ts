...
router.get('/callback', async (req, res) => {
  try {
    ...
    console.log('Facebook pages response:', {
      ...
      pages: pages && pages.length ? pages.map((p: any) => {
...
router.get('/forms', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    console.log('🔍 Buscando formulários do Facebook:', {
      ...
      accessTokenLength: access_token ? (access_token as string).length : 0
    });
    ...
    let tokenToUse = access_token as string;
    ...
    if (!tokenToUse && user_access_token) {
      ...
      try {
        const pagesResponse = await axios.get(
          ...
          {
            params: {
              access_token: user_access_token as string,
      ...
      if ((errorCode === 200 || errorCode === 100) && user_access_token && tokenSource === 'provided') {
        ...
        try {
          ...
          console.log('🔍 Página encontrada para retry:', {
            ...
            userTokenPreview: user_access_token ? (user_access_token as string).substring(0, 20) + '...' : 'null',
            areTokensDifferent: page && page.access_token && user_access_token ? page.access_token !== (user_access_token as string) : false
          });
          
          if (page && page.access_token) {
            // Verificar se o token da página é diferente do token do usuário
            const isDifferentToken = page.access_token !== (user_access_token as string);
            console.log('🔑 Comparação de tokens no retry:', {
              pageTokenLength: page.access_token.length,
              userTokenLength: (user_access_token as string).length,
              areDifferent: isDifferentToken,
              pageTokenPreview: page.access_token.substring(0, 20) + '...',
              userTokenPreview: (user_access_token as string).substring(0, 20) + '...'
            });
            
            // Tentar verificar permissões do token usando debug_token
            try {
              const debugResponse = await axios.get(
                `${FACEBOOK_API_BASE}/debug_token`,
                {
                  params: {
  ...
  try {
    ...
    try {
      const usersResponse = await axios.get(
        `${FACEBOOK_API_BASE}/${page_id}/roles`,
        {
          params: {
            access_token: access_token as string
          }
